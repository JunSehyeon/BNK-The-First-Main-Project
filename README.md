...
| 이름 | 역할 | 주요 업무 |
|------|------|-----------|
| **손진일** | 팀장 | 실시간 증권(REST+WS), 마이페이지 ETF, 관리자, 멀티스레드 스케줄러, 시스템 통합 |
| **전세현** | 팀원 | 외부 API 파이프라인(전체 수집) , 환율 API, 퇴직연금 IRP 계좌 개설, 마이데이터 비교 서비스, 고객센터 |
...
### 10-3. 전세현 – 외부 API 수집 · 환율 · 마이데이터 비교 · 고객센터/관리자 개발 · UX/UI 공동 기획

#### 🔹 외부 API 연동(수집 파이프라인)

- **프로젝트에 사용된 모든 외부 API Key 수집 및 관리 전담**
  - Korea Exim 환율 API  
  - DB증권 REST API
  - 키움증권 REST API

- **Korea Exim 환율 API**
  - `authkey`, `searchdate`, `data=AP01` 기반으로 환율 JSON 수신  
  - 주말/공휴일은 직전 영업일로 자동 보정  
  - 당일 데이터 비어 있으면 최근 유효 데이터 fallback 로직 구현  
  - `FxService`에서 가공 후 Controller → View로 전달해 USD/KRW 환율 실시간 표시  

---

#### 🔹 마이데이터 기반 연금상품 비교 서비스 구현

- `MyDataAccount` 테이블 설계 및 더미 데이터 구성  
  - 은행명, 운용사, 상품명, 위험등급, 수익률, 가입금액, 평가금액 등 포함  

- **CompareService**
  - 타행 계좌 평균 수익률 계산 → 기준 수익률 산정  
  - 타입·위험등급 기반 후보상품 필터링  
  - 사용자 보유상품 대비 더 높은 수익률 상품만 추천 리스트로 추출  

- **CompareController**
  - CompareService 결과를 View로 전달  
  - 타행 IRP/연금 vs BNK 상품 비교 화면 구성  

---

#### 🔹 고객센터(FAQ·자료실·Q&A) 전체 화면 및 기능 개발

- 고객센터 메인, FAQ, 자료실, Q&A UI/UX 구성  
- 게시글 목록·검색·페이지네이션·상세 보기 전체 구현  
- Q&A 문의 시 로그인/비로그인 사용자 분기 처리  
- 최신순 정렬, 문서 다운로드 등 실사용 패턴을 고려한 기능 구현  

---

#### 🔹 관리자 페이지 – 고객센터 CRUD 구축

- 관리자 전용 게시글 **등록/수정/삭제/조회** 기능 개발  
- 첨부파일 업로드 처리  
- 사용자 버전과 독립된 관리자 전용 UI 스타일 구성  
- 고객센터 도메인은 사용자 + 관리자 **전 영역 단독 개발**  

---

#### 🔹 UX/UI 디자인 & 서비스 컨셉 기획(조지영 팀원과 공동작업)

- 서비스 전체 **IA(Information Architecture)** 설계  
- 사용자 흐름 기반 UX 플로우 정의  
- Figma를 활용한 와이어프레임·프로토타입 제작  
- 전체 페이지 공통 컴포넌트 구조 및 UI 규칙 정리  
- 서비스 컨셉 주도  
